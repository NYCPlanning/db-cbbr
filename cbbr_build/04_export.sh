...
# points
pgsql2shp -u $DBUSER -f cbbr_build/output/cbbr_submissions_pts $DBNAME "SELECT * FROM cbbr_submissions WHERE ST_GeometryType(geom)='ST_MultiPoint' AND borough IS NOT NULL"

# polygons
pgsql2shp -u $DBUSER -f cbbr_build/output/cbbr_submissions_poly $DBNAME "SELECT * FROM cbbr_submissions WHERE ST_GeometryType(geom)='ST_MultiPolygon' AND borough IS NOT NULL"
