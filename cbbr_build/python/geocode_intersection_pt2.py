...
# read in cbbr table
cbbr = pd.read_sql_query('SELECT * FROM cbbr_submissions WHERE streetsegment IS NULL AND streetcross1 IS NOT NULL AND streetcross2 IS NOT NULL AND borough IS NOT NULL AND geom IS NULL;', engine)
...
for i in range(len(cbbr)):
    new = get_loc(cbbr['streetcross1'][i],
                  cbbr['streetcross2'][i],
                  cbbr['borough'][i]
    )
...
for i in range(len(cbbr)):
    if (locs['lat'][i] != 'none') & (locs['lon'][i] != 'none'):
        upd = "UPDATE cbbr_submissions a SET geom = ST_SetSRID(ST_MakePoint(" + str(locs['lon'][i]) + ", " + str(locs['lat'][i]) + "), 4326), geomsource = 'geoclient', dataname='lat long', datasource='doitt' WHERE streetcross1 = '" + cbbr['streetcross1'][i] + "' AND a.streetcross2 = '"+ cbbr['streetcross2'][i] + "';"
        engine.execute(upd)
